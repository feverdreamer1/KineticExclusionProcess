program kep
    ...
    real(dp), parameter :: beta = 1.0_dp ! Energy exponent f(e) = e^beta
...
contains
    ...
    subroutine perform_kmc_step()
        ...
        if (iaux == 2) then
            ...
            ! Jump (Particle-Hole)
            ! Esto solo puede ocurrir en el BULK (1..L).
    ...
    subroutine take_measurement()
        implicit none
        integer :: current_N  ! Variable nueva local

        ! --- AÑADIR ESTE CHEQUEO AL PRINCIPIO ---
        current_N = sum(occupation(1:L))
        if (current_N /= N) then
            write(*,*) "!!! ERROR CRITICO !!! N ha cambiado."
            write(*,*) "Step:", sample_count, " N actual:", current_N, " N esperado:", N
            stop
        end if
        ! ----------------------------------------
