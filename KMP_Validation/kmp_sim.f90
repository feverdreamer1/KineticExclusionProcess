!===============================================================================
! KMP_SIM.F90
! KMP Model Validation Simulation
! Based on KEP code with KMP limit parameters:
!   - beta = 0.0 (constant collision rate, recovers KMP)
!   - rho_left = rho_right = 1.0 (saturated reservoirs, N = L)
!   - Lattice initialized full (occupation = 1 for all internal sites)
!===============================================================================
...
program kmp_sim
    ...
    ! Physical parameters - KMP LIMIT
    integer, parameter :: L = 200
    integer, parameter :: N = L  ! KMP: N = L (full lattice)
    real(dp), parameter :: T_left = 4.0_dp
    real(dp), parameter :: T_right = 1.0_dp
    real(dp), parameter :: beta = 0.0_dp  ! KMP: constant collision rate f(e) = 1
    
    ! Reservoirs (for reference, not used for injection in thermal walls case)
    real(dp), parameter :: rho_left = 1.0_dp   ! KMP: saturated
    real(dp), parameter :: rho_right = 1.0_dp  ! KMP: saturated
    ...
    real(dp) :: temperature(0:L+1) ! Theoretical profile for initialization
    ...
    ! Arrays for Second Moments <x^2> for Error Calculation
    ...
    integer(int64) :: step
    real(dp) :: dt, random_val, pair_energy, local_temp, temp_energy
    integer :: temp_occ
    ...
    write(*,*) '========================================'
    write(*,*) '        KMP VALIDATION SIMULATION       '
    write(*,*) '========================================'
    write(*,*) 'Lattice size L = ', L
    write(*,*) 'Number of particles N = ', N, ' (KMP: N = L)'
    write(*,*) 'Left temperature T_L = ', T_left
    write(*,*) 'Right temperature T_R = ', T_right
    write(*,*) 'Beta (energy exponent) = ', beta, ' (KMP: beta = 0)'
    write(*,*) 'Rho_left = ', rho_left, ' (KMP: saturated)'
    write(*,*) 'Rho_right = ', rho_right, ' (KMP: saturated)'
...
contains
    ...
    subroutine initialize_lattice()
        ...
        ! KMP: Initialize FULL lattice (all sites occupied)
        do i = 1, L
            occupation(i) = 1
            ! Initialize energy with Boltzmann distribution at local temperature
            energy(i) = -temperature(i) * log(max(get_random(), 1.0e-10_dp))
        end do

        ! Fisher-Yates shuffle (internal sites only) - randomize energy distribution
        do i = L, 2, -1
        ...
        ! Initialize boundary conditions (Thermal Walls)
        ...
        write(*,*) 'Lattice initialized. Particles: ', n_particles, ' (should be ', L, ')'
    ...
    subroutine perform_kmc_step()
        ...
        if (iaux == 2) then
            ! Collision (Energy exchange) - KMP: only process in full lattice
            pair_energy = energy(i) + energy(j)
            energy(i) = get_random() * pair_energy
            energy(j) = pair_energy - energy(i)
            
        else if (iaux == 1) then
            ! Jump (Particle-Hole) - Should not occur in KMP (full lattice)
            ! But kept for code consistency
            pair_energy = energy(i) + energy(j)
            iaux2 = occupation(i)
            occupation(i) = occupation(j)
            occupation(j) = iaux2
    ...
    subroutine calculate_transition_rates()
        ...
        ! Bond 0 (Left Wall <-> Site 1):
        ! If site 1 is empty, rate = 0 (Wall cannot jump in).
        ! If site 1 occupied, rate = f(E_wall + E_1) = (E_wall + E_1)^beta
        ! For KMP (beta=0): rate = 1 (constant)
        if (occupation(1) == 0) then
            transition_rates(0) = 0.0_dp
        else
            transition_rates(0) = (energy(0) + energy(1))**beta
            ! For beta=0, this is 1.0, but we compute it for generality
        end if
        ...
        ! Bond L (Site L <-> Right Wall):
        ! If site L is empty, rate = 0
        if (occupation(L) == 0) then
            transition_rates(L) = total_rate
        else
    ...
    subroutine update_boundaries()
        ...
        ! Thermal Walls Implementation
        ! 1. Wall sites are ALWAYS occupied (act as fixed particles)
        occupation(0) = 1
        occupation(L+1) = 1
        
        ! 2. Their energy is reset randomly following Boltzmann distribution (Exponential)
        ! E ~ -T * ln(rand)
    ...
    subroutine take_measurement()
        ...
        do i = 1, L
            ...
            ! Moment 2: <x^2> for Error calculation
    ...
    subroutine calculate_final_stats()
        ...
        if (sample_count > 0) then
            ...
            do i = 1, L
                ...
                ! For KMP (full lattice, rho=1): T = e
